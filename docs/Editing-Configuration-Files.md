 ...
 * **trash_can_enabled:** Boolean (default = true) When deleting a torrent's downloaded files, move them to the system trash / recycle bin when possible instead of deleting them immediately. Falls back to deletion when no trash is available (e.g. on some headless or networked filesystems).
